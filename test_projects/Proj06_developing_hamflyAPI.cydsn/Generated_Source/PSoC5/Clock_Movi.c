...
/*******************************************************************************
* Function Name: Clock_Movi_Start
********************************************************************************
*
* Summary:
...
void Clock_Movi_Start(void) 
{
    ...
    Clock_Movi_CLKEN |= Clock_Movi_CLKEN_MASK;
	Clock_Movi_CLKSTBY |= Clock_Movi_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Clock_Movi_Stop
********************************************************************************
*
* Summary:
...
void Clock_Movi_Stop(void) 
{
    ...
    Clock_Movi_CLKEN &= (uint8)(~Clock_Movi_CLKEN_MASK);
	Clock_Movi_CLKSTBY &= (uint8)(~Clock_Movi_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clock_Movi_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Clock_Movi_CLKEN & Clock_Movi_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Clock_Movi_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock_Movi_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clock_Movi__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clock_Movi_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_Movi_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clock_Movi_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Clock_Movi_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Clock_Movi_GetSourceRegister();
    uint16 oldDivider = Clock_Movi_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock_Movi_CLKEN & Clock_Movi_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clock_Movi_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Clock_Movi_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Clock_Movi_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_Movi_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_Movi_MOD_SRC |= modeBitMask & (uint8)Clock_Movi_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_Movi_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_Movi_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clock_Movi_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clock_Movi_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_Movi_MOD_SRC & (uint8)(Clock_Movi_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clock_Movi_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Clock_Movi_GetDividerRegister();
    uint8 oldSrc = Clock_Movi_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clock_Movi_MOD_SRC |= CYCLK_SSS;
        Clock_Movi_MOD_SRC =
            (Clock_Movi_MOD_SRC & (uint8)(~Clock_Movi_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_Movi_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_Movi_MOD_SRC & Clock_Movi_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock_Movi_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Clock_Movi_PHASE = clkPhase & Clock_Movi_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock_Movi_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Clock_Movi_PHASE & Clock_Movi_PHASE_MASK;
}
