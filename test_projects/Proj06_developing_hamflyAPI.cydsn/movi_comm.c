...
/* Identical to movi_comm_request_attr() but also records the raw bytes
 * written to UART into tx_buf[0..tx_buf_max-1].  *tx_len_out receives
 * the number of bytes captured (capped at tx_buf_max).
 * Used by MOVI_SCAN mode to print the exact wire frame alongside the
 * response, so every line is self-contained for spreadsheet analysis.  */
movi_result_t movi_comm_request_attr_capture(movi_comm_t* m, uint16_t attr_id,
