...
/*******************************************************************************
* Function Name: EEPROM_CONFIG_Enable
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: EEPROM_CONFIG_Start
********************************************************************************
*
* Summary:
*  Starts EEPROM.
...
{
    EEPROM_CONFIG_Enable();
}


/*******************************************************************************
* Function Name: EEPROM_CONFIG_Stop
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: EEPROM_CONFIG_WriteByte
********************************************************************************
*
* Summary:
...
*  call EEPROM_CONFIG_UpdateTemperature() function if the temperature of the
    ...
    if (address < CY_EEPROM_SIZE)
    {
        ...
        if(CYRET_SUCCESS == CySpcLock())
        {
            status = CySpcLoadMultiByte(CY_SPC_FIRST_EE_ARRAYID, byteNumber, &dataByte, \
                                                                    EEPROM_CONFIG_SPC_BYTE_WRITE_SIZE);
...
/*******************************************************************************
* Function Name: EEPROM_CONFIG_ReadByte
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: EEPROM_CONFIG_UpdateTemperature
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: EEPROM_CONFIG_EraseSector
********************************************************************************
*
* Summary:
...
cystatus EEPROM_CONFIG_EraseSector(uint8 sectorNumber) 
{
    ...
    if(sectorNumber < (uint8) EEPROM_CONFIG_SECTORS_NUMBER)
    {
...
/*******************************************************************************
* Function Name: EEPROM_CONFIG_Write
********************************************************************************
*
* Summary:
...
*  EEPROM_CONFIG_UpdateTemperature() function if the temperature of the
...
/*******************************************************************************
* Function Name: EEPROM_CONFIG_StartWrite
********************************************************************************
*
* Summary:
...
*  EEPROM_CONFIG_Query(). EEPROM_CONFIG_Query() must be called
*  until it returns a status other than CYRET_STARTED. That indicates that the
*  write has completed. Until EEPROM_CONFIG_Query() detects that
...
*  you should call EEPROM_CONFIG_UpdateTemperature() API if the temperature
...
/*******************************************************************************
* Function Name: EEPROM_CONFIG_StartErase
********************************************************************************
*
* Summary:
...
*  must be used in combination with EEPROM_CONFIG_Query().
*  EEPROM_CONFIG_Query() must be called until it returns a status
*  other than CYRET_STARTED. That indicates the erase has been completed.
*  Until EEPROM_CONFIG_Query() detects that the erase is
...
/*******************************************************************************
* Function Name: EEPROM_CONFIG_Query
********************************************************************************
*
* Summary:
*  Checks the status of an earlier call to EEPROM_CONFIG_StartWrite() or
*  EEPROM_CONFIG_StartErase().
...
/*******************************************************************************
* Function Name: EEPROM_CONFIG_ByteWritePos
********************************************************************************
*
* Summary:
    ...
    if((rowNumber < (uint8) CY_EEPROM_NUMBER_ROWS) && (byteNumber < (uint8) SIZEOF_EEPROM_ROW))
    {
        ...
        if(CySpcLock() == CYRET_SUCCESS)
        {
            ...
            if(CySpcLoadMultiByte(CY_SPC_FIRST_EE_ARRAYID, (uint16)byteNumber, &dataByte,\
                                                                EEPROM_CONFIG_SPC_BYTE_WRITE_SIZE) == CYRET_STARTED)
            {
                while(CY_SPC_BUSY)
                {
                    /* Wait until SPC becomes idle */
