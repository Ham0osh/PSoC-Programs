 ...
 * Returns 1 on success, 0 if cal invalid or write failed. */
uint8 joystick_save(void)
{
    joy_cal_t cal;
    if (!joystick_get_cal(&cal)) return 0u;
    return psoc_eeprom_write(PSOC_EEPROM_SLOT_JOY_CAL,
                             &cal, (uint8)sizeof(joy_cal_t));
}
 
/* Load calibration from EEPROM and apply it.
 * Calls joystick_init internally with loaded cal.
 * invert_mask is re-applied from the caller's value.
 * Returns 1 if loaded and applied, 0 if no saved cal found. */
uint8 joystick_load(uint32 invert_mask)
{
    joy_cal_t cal;
    if (!psoc_eeprom_read(PSOC_EEPROM_SLOT_JOY_CAL,
                          &cal, (uint8)sizeof(joy_cal_t))) return 0u;
    if (!joystick_validate(&cal)) return 0u;
    joystick_init(&cal, invert_mask);
    return 1u;
}
