 ...
 * USBFS_DEBUG_GetDTERate(), USBFS_DEBUG_GetCharFormat(),
 * USBFS_DEBUG_GetParityType(), and USBFS_DEBUG_GetDataBits() APIs.
...
volatile uint8 USBFS_DEBUG_linesCoding[USBFS_DEBUG_MAX_MULTI_COM_NUM][USBFS_DEBUG_LINE_CODING_SIZE] =
{
    /*COM Port 1*/
    {
...
/**Used as a flag for the USBFS_DEBUG_IsLineChanged() API, to inform it that the
 ...
 * support. In case of 1 COM port, data is in 0 element.*/
volatile uint8  USBFS_DEBUG_linesChanged[USBFS_DEBUG_MAX_MULTI_COM_NUM];
 ...
 * USBFS_DEBUG_GetLineControl() API. It is an array of 2 elements for COM 
 * port 1 and COM port 2 for MultiCOM port support. In case of 1 COM port, data 
 * is in 0 element.*/
volatile uint16 USBFS_DEBUG_linesControlBitmap[USBFS_DEBUG_MAX_MULTI_COM_NUM];
/** Contains the 16-bit serial state value that was sent using the 
 * \ref USBFS_DEBUG_SendSerialState() API. . It is an array of 2 elements 
 * for COM port 1 and COM port 2 for MultiCOM port support. In case of 1 COM 
 * port, data is in 0 element.*/
volatile uint16 USBFS_DEBUG_serialStateBitmap[USBFS_DEBUG_MAX_MULTI_COM_NUM];
...
volatile uint8  USBFS_DEBUG_cdcDataInEp[USBFS_DEBUG_MAX_MULTI_COM_NUM];
 ...
 * port 2 for MultiCOM port support. In case of 1 COM port, data is in 0 element.*/
volatile uint8  USBFS_DEBUG_cdcDataOutEp[USBFS_DEBUG_MAX_MULTI_COM_NUM];
...
volatile uint8  USBFS_DEBUG_cdcCommInInterruptEp[USBFS_DEBUG_MAX_MULTI_COM_NUM];

/*PRIVATE*/

#define USBFS_DEBUG_CDC_IN_EP      (0u)
#define USBFS_DEBUG_CDC_OUT_EP     (1u)
#define USBFS_DEBUG_CDC_NOTE_EP    (2u)

#define USBFS_DEBUG_CDC_EP_MASK    (0x01u)

#define USBFS_DEBUG_GET_EP_COM_NUM(cdcComNums, epType) (((cdcComNums) >> (epType)) & USBFS_DEBUG_CDC_EP_MASK)
...
***************************************/
    ...
    static uint16 USBFS_DEBUG_StrLen(const char8 string[]) ;
    static t_USBFS_DEBUG_cdc_notification USBFS_DEBUG_serialStateNotification =
    {

        USBFS_DEBUG_SERIAL_STATE_REQUEST_TYPE, /* bRequestType    */
        USBFS_DEBUG_SERIAL_STATE,              /* bNotification   */
        0u,                                         /* wValue          */
        0u,                                         /* wValueMSB       */
        0u,                                         /* wIndex          */
        0u,                                         /* wIndexMSB       */
        USBFS_DEBUG_SERIAL_STATE_LENGTH,       /* wLength         */
        0u,                                         /* wLengthMSB      */
        0u,                                         /* wSerialState    */
        0u,                                         /* wSerialStateMSB */
    };
    static uint8 USBFS_DEBUG_activeCom = 0u;
#endif /* (USBFS_DEBUG_ENABLE_CDC_CLASS_API != 0u) */
...
/*******************************************************************************
* Function Name: USBFS_DEBUG_DispatchCDCClassRqst
****************************************************************************//**
*
...
*
* \globalvars
*   USBFS_DEBUG_linesCoding: Contains the current line coding structure.
...
*   USBFS_DEBUG_linesControlBitmap: Contains the current control signal
*     bitmap. It is set by the Host using SET_CONTROL_LINE request and returned
*     to the user code by the USBFS_GetLineControl() API.
*   USBFS_DEBUG_linesChanged: This variable is used as a flag for the
...
uint8 USBFS_DEBUG_DispatchCDCClassRqst(void) 
{
    ...
    comPort = USBFS_DEBUG_GetInterfaceComPort((uint8)USBFS_DEBUG_wIndexLoReg);
    ...
    if (0u != (USBFS_DEBUG_bmRequestTypeReg & USBFS_DEBUG_RQST_DIR_D2H))
    {
        ...
        {
            case USBFS_DEBUG_CDC_GET_LINE_CODING:
                USBFS_DEBUG_currentTD.count = USBFS_DEBUG_LINE_CODING_SIZE;
                USBFS_DEBUG_currentTD.pData = USBFS_DEBUG_linesCoding[comPort];
            ...
            #ifdef USBFS_DEBUG_DISPATCH_CDC_CLASS_CDC_READ_REQUESTS_CALLBACK
                requestHandled = USBFS_DEBUG_DispatchCDCClass_CDC_READ_REQUESTS_Callback();
            #endif /* (USBFS_DEBUG_DISPATCH_CDC_CLASS_CDC_READ_REQUESTS_CALLBACK) */
                break;
        }
    }
    else
    {
        ...
        switch (USBFS_DEBUG_bRequestReg)
        {
            case USBFS_DEBUG_CDC_SET_LINE_CODING:
                USBFS_DEBUG_currentTD.count  = USBFS_DEBUG_LINE_CODING_SIZE;
                USBFS_DEBUG_currentTD.pData  = USBFS_DEBUG_linesCoding[comPort];
                USBFS_DEBUG_linesChanged[comPort] |= USBFS_DEBUG_LINE_CODING_CHANGED;

                requestHandled = USBFS_DEBUG_InitControlWrite();
                break;

            case USBFS_DEBUG_CDC_SET_CONTROL_LINE_STATE:
                USBFS_DEBUG_linesControlBitmap[comPort] = (uint8) USBFS_DEBUG_wValueLoReg;
                USBFS_DEBUG_linesChanged[comPort]      |= USBFS_DEBUG_LINE_CONTROL_CHANGED;
            ...
            default:
                /* Do not handle this request unless callback is defined. */
            #ifdef USBFS_DEBUG_DISPATCH_CDC_CLASS_CDC_WRITE_REQUESTS_CALLBACK
                requestHandled = USBFS_DEBUG_DispatchCDCClass_CDC_WRITE_REQUESTS_Callback();
            #endif /* (USBFS_DEBUG_DISPATCH_CDC_CLASS_CDC_WRITE_REQUESTS_CALLBACK) */
                break;
        }
    }
...
/***************************************************************************
* Function Name: USBFS_DEBUG_GetInterfaceComPort
************************************************************************//**
*   \internal
...
uint8 USBFS_DEBUG_GetInterfaceComPort(uint8 interface) 
{
    ...
    while (comPort < USBFS_DEBUG_MAX_MULTI_COM_NUM)
    {
        notEp = USBFS_DEBUG_cdcCommInInterruptEp[comPort];

        if (USBFS_DEBUG_EP[notEp].interface == interface)
        {
            res = comPort;
            comPort = USBFS_DEBUG_MAX_MULTI_COM_NUM;
        }
...
#if (USBFS_DEBUG_ENABLE_CDC_CLASS_API != 0u)
/***************************************************************************
* Function Name: USBFS_DEBUG_CDC_Init
************************************************************************//**
*
...
*   USBFS_DEBUG_SUCCESS   CDC interface was initialized correctly
*   USBFS_DEBUG_FAILURE   CDC interface was not initialized
*
* \globalvars
*   USBFS_DEBUG_linesChanged: Initialized to zero.
*   USBFS_DEBUG_cdcDataOutEp: Used as an OUT endpoint number.
*
* \reentrant
*  No.
*
...
uint8 USBFS_DEBUG_CDC_Init(void) 
{
    uint8 comPort;
    uint8 outEp;
    uint8 ret = USBFS_DEBUG_SUCCESS;

    USBFS_DEBUG_activeCom = 0u;
    USBFS_DEBUG_linesChanged[USBFS_DEBUG_COM_PORT1] = 0u;
    USBFS_DEBUG_linesChanged[USBFS_DEBUG_COM_PORT2] = 0u;

    for(comPort = 0u; comPort<USBFS_DEBUG_MAX_MULTI_COM_NUM; comPort++)
    {
        outEp = USBFS_DEBUG_cdcDataOutEp[comPort];
        if((0u != outEp) && (USBFS_DEBUG_MAX_EP > outEp))
        {
            USBFS_DEBUG_EnableOutEP(outEp);
        }

    }

    /* COM Port 1 should be correct to proceed. */
    if ((0u == USBFS_DEBUG_cdcDataInEp[USBFS_DEBUG_COM_PORT1]) \
            || (0u == USBFS_DEBUG_cdcDataOutEp[USBFS_DEBUG_COM_PORT1]) \
            || (0u ==  USBFS_DEBUG_cdcCommInInterruptEp[USBFS_DEBUG_COM_PORT1])
            || (USBFS_DEBUG_cdcDataInEp[USBFS_DEBUG_COM_PORT1] >= USBFS_DEBUG_MAX_EP)
            || (USBFS_DEBUG_cdcDataOutEp[USBFS_DEBUG_COM_PORT1] >= USBFS_DEBUG_MAX_EP)
            || (USBFS_DEBUG_cdcCommInInterruptEp[USBFS_DEBUG_COM_PORT1] >= USBFS_DEBUG_MAX_EP))
    {
        ret = USBFS_DEBUG_FAILURE;
    }
...
/*******************************************************************************
* Function Name: USBFS_DEBUG_PutData
****************************************************************************//**
*
*  This function sends a specified number of bytes from the location specified
*  by a pointer to the PC. The USBFS_DEBUG_CDCIsReady() function should be
...
*  identified. To send zero-length packet, use USBFS_DEBUG_PutData() API
...
* \globalvars
*
*   USBFS_DEBUG_cdcDataInEp: CDC IN endpoint number used for sending
*     data.
*
* \reentrant
*  No.
...
{
    uint8 epNumber = USBFS_DEBUG_cdcDataInEp[USBFS_DEBUG_activeCom];

    /* Limit length to maximum packet size for endpoint. */
    if (length > USBFS_DEBUG_EP[epNumber].bufferSize)
    {
        /* Caution: Data will be lost if length is greater than Max Packet Size. */
        length = USBFS_DEBUG_EP[epNumber].bufferSize;

        /* Halt CPU in debug mode */
        CYASSERT(0u != 0u);
    }

    USBFS_DEBUG_LoadInEP(epNumber, pData, length);
}


/*******************************************************************************
* Function Name: USBFS_DEBUG_StrLen
****************************************************************************//**
*
...
/***************************************************************************
* Function Name: USBFS_DEBUG_PutString
************************************************************************//**
*
...
*  The USBFS_DEBUG_CDCIsReady() function should be called before
*  sending data with a new call to USBFS_DEBUG_PutString(), to be sure
...
void USBFS_DEBUG_PutString(const char8 string[]) 
{
    ...
    strLength = USBFS_DEBUG_StrLen(string);

    do
    {
        /* Limit length to maximum packet size of endpoint. */
        sendLength = (strLength > USBFS_DEBUG_EP[epNumber].bufferSize) ?
                                  USBFS_DEBUG_EP[epNumber].bufferSize : strLength;

        /* Load IN endpoint and expose it to host. */
        USBFS_DEBUG_LoadInEP(epNumber, (const uint8 *)&string[bufIndex], sendLength);
        strLength -= sendLength;

        /* If more data are present to send or full packet was sent */
        if ((strLength > 0u) || (sendLength == USBFS_DEBUG_EP[epNumber].bufferSize))
        {
            bufIndex += sendLength;

            /* Wait until host read data from IN endpoint buffer. */
            while (USBFS_DEBUG_IN_BUFFER_FULL == USBFS_DEBUG_EP[epNumber].apiEpState)
            {
            }
            ...
            */
            if (0u == strLength)
            {
                USBFS_DEBUG_LoadInEP(epNumber, NULL, 0u);
            }
        }
    }
    while (strLength > 0u);
...
/***************************************************************************
* Function Name: USBFS_DEBUG_PutChar
************************************************************************//**
*
...
void USBFS_DEBUG_PutChar(char8 txDataByte) 
{
    ...
    USBFS_DEBUG_LoadInEP(USBFS_DEBUG_cdcDataInEp[USBFS_DEBUG_activeCom], &dataByte, 1u);
}


/*******************************************************************************
* Function Name: USBFS_DEBUG_PutCRLF
****************************************************************************//**
*
...
void USBFS_DEBUG_PutCRLF(void) 
{
    ...
    USBFS_DEBUG_LoadInEP(USBFS_DEBUG_cdcDataInEp[USBFS_DEBUG_activeCom], (const uint8 *)txData, 2u);
}


/*******************************************************************************
* Function Name: USBFS_DEBUG_GetCount
****************************************************************************//**
*
*  This function returns the number of bytes that were received from the PC.
*  The returned length value should be passed to USBFS_DEBUG_GetData() as
*  a parameter to read all received data. If all of the received data is not
*  read at one time by the USBFS_DEBUG_GetData() API, the unread data will
*  be lost.
*
* \return
...
*
* \globalvars
*   USBFS_DEBUG_cdcDataOutEp: CDC OUT endpoint number used.
*
*******************************************************************************/
uint16 USBFS_DEBUG_GetCount(void) 
{
    ...
    uint8  epNumber = USBFS_DEBUG_cdcDataOutEp[USBFS_DEBUG_activeCom];

    if (USBFS_DEBUG_OUT_BUFFER_FULL == USBFS_DEBUG_EP[epNumber].apiEpState)
    {
        bytesCount = USBFS_DEBUG_GetEPCount(epNumber);
    }
    else
    {
        bytesCount = 0u;
...
/*******************************************************************************
* Function Name: USBFS_DEBUG_DataIsReady
****************************************************************************//**
*
*  This function returns a non-zero value if the component received data or
*  received zero-length packet. The USBFS_DEBUG_GetAll() or
*  USBFS_DEBUG_GetData() API should be called to read data from the buffer
...
{
    return (USBFS_DEBUG_GetEPState(USBFS_DEBUG_cdcDataOutEp[USBFS_DEBUG_activeCom]));
}


/*******************************************************************************
* Function Name: USBFS_DEBUG_CDCIsReady
****************************************************************************//**
*
...
*  USBFS_DEBUG_PutData(),USBFS_DEBUG_PutString(),
*  USBFS_DEBUG_PutChar or USBFS_DEBUG_PutCRLF(),
...
*
* \globalvars
*   USBFS_DEBUG_cdcDataInEp: CDC IN endpoint number used.
*
*******************************************************************************/
uint8 USBFS_DEBUG_CDCIsReady(void) 
{
    return (USBFS_DEBUG_GetEPState(USBFS_DEBUG_cdcDataInEp[USBFS_DEBUG_activeCom]));
}


/***************************************************************************
* Function Name: USBFS_DEBUG_GetData
************************************************************************//**
*
...
*  The USBFS_DEBUG_DataIsReady() API should be called first, to be sure
*  that data is received from the host. If all received data will not be read at
*  once, the unread data will be lost. The USBFS_DEBUG_GetData() API should
...
uint16 USBFS_DEBUG_GetData(uint8* pData, uint16 length) 
{
    ...
    length = USBFS_DEBUG_ReadOutEP(epNumber, pData, length);

#if (USBFS_DEBUG_EP_MANAGEMENT_DMA_MANUAL)
    /* Wait until DMA complete transferring data from OUT endpoint buffer. */
    while (USBFS_DEBUG_OUT_BUFFER_FULL == USBFS_DEBUG_GetEPState(epNumber))
    {
    }
...
*
* \globalvars
*   - \ref USBFS_DEBUG_cdcDataOutEp: CDC OUT endpoint number used.
*   - \ref USBFS_DEBUG_EP[].bufferSize: EP max packet size is used as a
...
uint16 USBFS_DEBUG_GetAll(uint8* pData) 
{
    ...
    /* Read data from OUT endpoint buffer. */
    dataLength = USBFS_DEBUG_ReadOutEP(epNumber, pData, USBFS_DEBUG_EP[epNumber].bufferSize);
...
/***************************************************************************
* Function Name: USBFS_DEBUG_GetChar
************************************************************************//**
*
...
uint8 USBFS_DEBUG_GetChar(void) 
{
    ...
    (void) USBFS_DEBUG_ReadOutEP(epNumber, &rxData, 1u);
...
/*******************************************************************************
* Function Name: USBFS_DEBUG_IsLineChanged
****************************************************************************//**
*
...
*  device. The USBFS_DEBUG_GetDTERate(), USBFS_DEBUG_GetCharFormat()
*  or USBFS_DEBUG_GetParityType() or USBFS_DEBUG_GetDataBits() API
*  should be called to read data coding information.
*  The USBFS_DEBUG_GetLineControl() API should be called to read line
*  control information.
*
* \return
...
*
* \globalvars
*  - \ref USBFS_DEBUG_transferState: it is checked to be sure then OUT
*    data phase has been complete, and data written to the lineCoding or
*    Control Bitmap buffer.
*  - \ref USBFS_DEBUG_linesChanged: used as a flag to be aware that
...
uint8 USBFS_DEBUG_IsLineChanged(void) 
{
    ...
    if (USBFS_DEBUG_transferState == USBFS_DEBUG_TRANS_STATE_IDLE)
    {
        if (USBFS_DEBUG_linesChanged[USBFS_DEBUG_activeCom] != 0u)
        {
            state = USBFS_DEBUG_linesChanged[USBFS_DEBUG_activeCom];
            USBFS_DEBUG_linesChanged[USBFS_DEBUG_activeCom] = 0u;
        }
    }
...
/***************************************************************************
* Function Name: USBFS_DEBUG_GetDTERate
************************************************************************//**
*
...
*
* \globalvars
*  USBFS_DEBUG_linesCoding: First four bytes converted to uint32
...
uint32 USBFS_DEBUG_GetDTERate(void) 
{
    ...
    rate = USBFS_DEBUG_linesCoding[USBFS_DEBUG_activeCom][USBFS_DEBUG_LINE_CODING_RATE + 3u];
    rate = (rate << 8u) | USBFS_DEBUG_linesCoding[USBFS_DEBUG_activeCom][USBFS_DEBUG_LINE_CODING_RATE + 2u];
    rate = (rate << 8u) | USBFS_DEBUG_linesCoding[USBFS_DEBUG_activeCom][USBFS_DEBUG_LINE_CODING_RATE + 1u];
    rate = (rate << 8u) | USBFS_DEBUG_linesCoding[USBFS_DEBUG_activeCom][USBFS_DEBUG_LINE_CODING_RATE];
...
/*******************************************************************************
* Function Name: USBFS_DEBUG_GetCharFormat
****************************************************************************//**
*
...
*
* \globalvars
*  USBFS_DEBUG_linesCoding: used to get a parameter.
*
*******************************************************************************/
uint8 USBFS_DEBUG_GetCharFormat(void) 
{
    return (USBFS_DEBUG_linesCoding[USBFS_DEBUG_activeCom][USBFS_DEBUG_LINE_CODING_STOP_BITS]);
}


/*******************************************************************************
* Function Name: USBFS_DEBUG_GetParityType
****************************************************************************//**
*
...
{
    return (USBFS_DEBUG_linesCoding[USBFS_DEBUG_activeCom][USBFS_DEBUG_LINE_CODING_PARITY]);
}


/***************************************************************************
* Function Name: USBFS_DEBUG_GetDataBits
************************************************************************//**
*
...
{
    return (USBFS_DEBUG_linesCoding[USBFS_DEBUG_activeCom][USBFS_DEBUG_LINE_CODING_DATA_BITS]);
}


/***************************************************************************
* Function Name: USBFS_DEBUG_GetLineControl
************************************************************************//**
*
...
*
* \globalvars
*  USBFS_DEBUG_linesControlBitmap: used to get a parameter.
*
*******************************************************************************/
uint16 USBFS_DEBUG_GetLineControl(void) 
{
    return (USBFS_DEBUG_linesControlBitmap[USBFS_DEBUG_activeCom]);
}


/*******************************************************************************
* Function Name: USBFS_DEBUG_SendSerialState
****************************************************************************//**
*
...
*  USBFS_DEBUG_NotificationIsReady() API must be called to check if the
...
{
    uint8 epNumber = USBFS_DEBUG_cdcCommInInterruptEp[USBFS_DEBUG_activeCom];

    if(USBFS_DEBUG_SERIAL_STATE_SIZE <= USBFS_DEBUG_EP[epNumber].bufferSize)
    {
        /* Save current SERIAL_STATE bitmap. */
        USBFS_DEBUG_serialStateBitmap[USBFS_DEBUG_activeCom] = serialState;

        /* Add interface number */
        USBFS_DEBUG_serialStateNotification.wIndex = USBFS_DEBUG_EP[epNumber].interface;

        /*Form SERIAL_STATE data*/
        USBFS_DEBUG_serialStateNotification.wSerialState =    LO8(USBFS_DEBUG_serialStateBitmap[USBFS_DEBUG_activeCom]);
        USBFS_DEBUG_serialStateNotification.wSerialStateMSB = HI8(USBFS_DEBUG_serialStateBitmap[USBFS_DEBUG_activeCom]);

        USBFS_DEBUG_LoadInEP(epNumber, (uint8 *) &USBFS_DEBUG_serialStateNotification, sizeof(USBFS_DEBUG_serialStateNotification));
    }
}


/*******************************************************************************
* Function Name: USBFS_DEBUG_GetSerialState
****************************************************************************//**
*
...
{
    return USBFS_DEBUG_serialStateBitmap[USBFS_DEBUG_activeCom];
}


/*******************************************************************************
* Function Name: USBFS_DEBUG_NotificationIsReady
****************************************************************************//**
*
...
*  USBFS_DEBUG_SendSerialState() to ensure that any previous
...
{
    return (USBFS_DEBUG_EP[USBFS_DEBUG_cdcCommInInterruptEp[USBFS_DEBUG_activeCom]].apiEpState);
}


/*******************************************************************************
* Function Name: USBFS_DEBUG_SetComPort
****************************************************************************//**
*
...
{
    if ((USBFS_DEBUG_activeCom != comNumber) && \
            (comNumber < USBFS_DEBUG_MAX_MULTI_COM_NUM ))
    {
        USBFS_DEBUG_activeCom = comNumber;
    }
}


/*******************************************************************************
* Function Name: USBFS_DEBUG_GetComPort
****************************************************************************//**
*
...
/***************************************************************************
* Function Name: USBFS_DEBUG_Cdc_EpInit
************************************************************************//**
*
*  \internal
...
*   USBFS_DEBUG_cdcDataInEp[], USBFS_DEBUG_cdcCommInInterruptEp[],
*   USBFS_DEBUG_cdcDataOutEp[]
*
...
uint8 USBFS_DEBUG_Cdc_EpInit(const T_USBFS_DEBUG_EP_SETTINGS_BLOCK CYCODE *pEP, uint8 epNum, uint8 cdcComNums) 
{
    ...
    {
        if (epType != USBFS_DEBUG_EP_TYPE_INT)
        {
            USBFS_DEBUG_cdcDataInEp[USBFS_DEBUG_GET_EP_COM_NUM(cdcComNums, USBFS_DEBUG_CDC_IN_EP)] = epNum;
            cdcComNums |= (uint8)(USBFS_DEBUG_COM_PORT2 << USBFS_DEBUG_CDC_IN_EP);
        }
        else
        {

            USBFS_DEBUG_cdcCommInInterruptEp[USBFS_DEBUG_GET_EP_COM_NUM(cdcComNums, USBFS_DEBUG_CDC_NOTE_EP)] = epNum;
            cdcComNums |= (uint8)(USBFS_DEBUG_COM_PORT2 << USBFS_DEBUG_CDC_NOTE_EP);
        }
    }
    else
    {
        if (epType != USBFS_DEBUG_EP_TYPE_INT)
        {
            USBFS_DEBUG_cdcDataOutEp[USBFS_DEBUG_GET_EP_COM_NUM(cdcComNums, USBFS_DEBUG_CDC_OUT_EP)] = epNum;
            cdcComNums |= (uint8)(USBFS_DEBUG_COM_PORT2 << USBFS_DEBUG_CDC_OUT_EP);
        }
    }
    return (cdcComNums);
}
