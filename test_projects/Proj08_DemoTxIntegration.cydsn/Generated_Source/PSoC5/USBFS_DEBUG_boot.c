...
static uint8  USBFS_DEBUG_started = 0u;


/*******************************************************************************
* Function Name: USBFS_DEBUG_CyBtldrCommStart
****************************************************************************//**
*
...
void USBFS_DEBUG_CyBtldrCommStart(void) 
{
    ...
    USBFS_DEBUG_Start(0u, USBFS_DEBUG_DWR_POWER_OPERATION);

    /* USB component started, the correct enumeration will be checked in the first Read operation. */
    USBFS_DEBUG_started = 1u;
}


/*******************************************************************************
* Function Name: USBFS_DEBUG_CyBtldrCommStop.
****************************************************************************//**
*
...
/*******************************************************************************
* Function Name: USBFS_DEBUG_CyBtldrCommReset.
****************************************************************************//**
*
...
{
    USBFS_DEBUG_EnableOutEP(USBFS_DEBUG_BTLDR_OUT_EP); 
}


/*******************************************************************************
* Function Name: USBFS_DEBUG_CyBtldrCommWrite.
****************************************************************************//**
*
                                                            ...
                                                            
{
    ...
    USBFS_DEBUG_LoadInEP(USBFS_DEBUG_BTLDR_IN_EP, pData, USBFS_DEBUG_BTLDR_SIZEOF_READ_BUFFER);

    /* Wait unitl host reads data from IN endpoint. */
    while ((USBFS_DEBUG_GetEPState(USBFS_DEBUG_BTLDR_IN_EP) == USBFS_DEBUG_IN_BUFFER_FULL) &&
           (0u != timeoutMs))
    {
        CyDelay(USBFS_DEBUG_BTLDR_WAIT_1_MS);
        timeoutMs--;
    }

    if (USBFS_DEBUG_GetEPState(USBFS_DEBUG_BTLDR_IN_EP) == USBFS_DEBUG_IN_BUFFER_FULL)
    {
...
/*******************************************************************************
* Function Name: USBFS_DEBUG_CyBtldrCommRead.
****************************************************************************//**
*
                                                            ...
                                                            
{
    ...
    if (size > USBFS_DEBUG_BTLDR_SIZEOF_WRITE_BUFFER)
    {
        size = USBFS_DEBUG_BTLDR_SIZEOF_WRITE_BUFFER;
    }

    /* Wait for enumeration first time. */
    if (0u != USBFS_DEBUG_started)
    {
        /* Wait for device enumeration. */
        while ((0u == USBFS_DEBUG_GetConfiguration()) && (0u != timeoutMs))
        {
            CyDelay(USBFS_DEBUG_BTLDR_WAIT_1_MS);
            timeoutMs--;
        }
        ...
        if (0u != USBFS_DEBUG_GetConfiguration())
        {
            (void) USBFS_DEBUG_IsConfigurationChanged();  /* Clear configuration changes state status. */
            USBFS_DEBUG_CyBtldrCommReset();
            
            USBFS_DEBUG_started = 0u;
        }
    }
    else /* Check for configuration changes, has been done by Host. */
    {
        if (0u != USBFS_DEBUG_IsConfigurationChanged()) /* Host could send double SET_INTERFACE request or RESET. */
        {
            if (0u != USBFS_DEBUG_GetConfiguration())   /* Init OUT endpoints when device reconfigured. */
            {
                USBFS_DEBUG_CyBtldrCommReset();
            }
        }
    ...
    while ((USBFS_DEBUG_GetEPState(USBFS_DEBUG_BTLDR_OUT_EP) != USBFS_DEBUG_OUT_BUFFER_FULL) && \
           (0u != timeoutMs))
    {
        CyDelay(USBFS_DEBUG_BTLDR_WAIT_1_MS);
        timeoutMs--;
    ...
    if (USBFS_DEBUG_GetEPState(USBFS_DEBUG_BTLDR_OUT_EP) == USBFS_DEBUG_OUT_BUFFER_FULL)
    {
        *count = USBFS_DEBUG_ReadOutEP(USBFS_DEBUG_BTLDR_OUT_EP, pData, size);
        retCode = CYRET_SUCCESS;
    }
    else
    {
