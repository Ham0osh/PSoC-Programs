...
typedef struct {
    ...
    // GPS telemetry (attribute 4) — valid only when gps_valid == true
    bool  gps_valid;
    float gps_lat_deg;       // degrees, +N/-S
    float gps_lon_deg;       // degrees, +E/-W
    float gps_alt_m;         // metres MSL
    float gps_ground_spd_ms; // m/s
    float gps_heading_deg;   // degrees true
    float gps_hacc_m;        // horizontal accuracy (m)
    float gps_vacc_m;        // vertical accuracy (m)
    float gps_sacc_ms;       // speed accuracy (m/s)
} movi_status_t;
...
typedef struct {
    ...
    uint32_t gps_poll_ms;   // timestamp of last GPS poll (QX_GetTicks_ms units)
} movi_comm_t;
