                    ...
                    ff_api_control_type_e* ctrl_mode)
{
    ...
    *ctrl_mode       = DEFER;
    joystick_on_key('x');
    UART_DEBUG_PutString("[CLI] All stopped  Mode: DEFR  -> STANDBY\r\n");
}
                                 ...
                                 uint16_t rxlen, const uint8_t* p, uint8_t plen)
{
    ...
    if (plen < 6u) { UART_DEBUG_PutString("  ERROR: too short\r\n"); return; }
    ...
    if (plen >= 19u) {
        UART_DEBUG_PutString("  -- rates (off 9-17) --\r\n");
                            ...
                            uint16_t rxlen, const uint8_t* p, uint8_t plen)
{
    ...
    UART_DEBUG_PutString(buf);
    print_hex_dump(p, plen);
    if (plen < 12u) { UART_DEBUG_PutString("  ERROR: too short\r\n"); return; }
                             ...
                             uint16_t rxlen, const uint8_t* p, uint8_t plen)
{
    ...
    UART_DEBUG_PutString(buf);
    print_hex_dump(p, plen);
    if (plen < 4u) { UART_DEBUG_PutString("  ERROR: too short\r\n"); return; }
                                ...
                                uint16_t rxlen, const uint8_t* p, uint8_t plen)
{
    ...
    UART_DEBUG_PutString(buf);
    print_hex_dump(p, plen);
    if (plen < 5u) { UART_DEBUG_PutString("  ERROR: too short\r\n"); return; }
    ...
    UART_DEBUG_PutString("  (* = confirmed against iOS app)\r\n");
}
                                     ...
                                     uint16_t rxlen, const uint8_t* p, uint8_t plen)
{
    ...
    UART_DEBUG_PutString(buf);
    print_hex_dump(p, plen);
    if (plen < 7u) { UART_DEBUG_PutString("  ERROR: too short\r\n"); return; }
    ...
    UART_DEBUG_PutString("  NOTE: This is GCU box orientation, NOT stabilised camera.\r\n");
    UART_DEBUG_PutString("        Rotate the whole unit to watch yaw track physical heading.\r\n");
}
                                    ...
                                    uint16_t rxlen, const uint8_t* p, uint8_t plen)
{
    ...
    UART_DEBUG_PutString(buf);
    print_hex_dump(p, plen);
    if (plen < 9u) { UART_DEBUG_PutString("  ERROR: too short\r\n"); return; }
...
static void print_quat_crosscheck(movi_comm_t* m)
{
    ...
    UART_DEBUG_PutString("[TEST3] Step 1: attr 287 auto-push (last received) -> Euler via PSoC math\r\n");

    if (!st.valid) {
        UART_DEBUG_PutString("[TEST3] ERROR: no valid attr 287 frame yet -- enter MOVI_CONTROL first\r\n");
        return;
    }
    ...
    UART_DEBUG_PutString("[TEST3] Step 2: requesting attr 22 (direct Euler from GCU) ...\r\n");
    ...
    UART_DEBUG_PutString("[TEST3] When attr 22 arrives, compare yaw/pitch/roll values above.\r\n");
    UART_DEBUG_PutString("[TEST3] They should agree within ~0.5 deg if both decodes are correct.\r\n");
}
...
int main(void)
{
    ...
    UART_DEBUG_PutString("========================================\r\n");
    UART_DEBUG_PutString("  m=ctrl  t=test  r=RESET  v=SENSOR  w=scan\r\n");
    UART_DEBUG_PutString("  j=joy  l=mode  s=adc  c=cal  k=kill  x=quiet\r\n");
    UART_DEBUG_PutString("  -- MOVI_CONTROL extra keys --\r\n");
    UART_DEBUG_PutString("  f=attr48  p=platform-yaw toggle\r\n");
    UART_DEBUG_PutString("  0=home  1=pan+45  2=pan-45  3=tilt+30  4=tilt-30  5=pan+90\r\n");
    UART_DEBUG_PutString("  (abs presets require 'l' to ABS mode first)\r\n");
    UART_DEBUG_PutString("  -- TEST mode keys --\r\n");
    UART_DEBUG_PutString("  a=att22  g=GPS  b=baro  s=stat  n=mag\r\n");
    UART_DEBUG_PutString("  2=platform-att  4=motor-drv  q=quat-crosscheck\r\n");
    UART_DEBUG_PutString("  t=exit  (2s timeout)  k/x=kill+exit\r\n");
    UART_DEBUG_PutString("========================================\r\n");
    ...
    UART_DEBUG_PutString("[Init] Mode: STANDBY\r\n");
    ...
    for (;;)
    {
        ...
        if (app_mode == APP_MODE_STANDBY || app_mode == APP_MODE_MOVI_CONTROL)
        {
            if (ch == 'm' || ch == 'M')
            {
                if (app_mode == APP_MODE_STANDBY) {
                    ...
                    UART_DEBUG_PutString("[CLI] -> MOVI_CONTROL  TX active\r\n");
                } else {
                    app_mode = APP_MODE_STANDBY;
                    UART_DEBUG_PutString("[CLI] -> STANDBY  TX stopped\r\n");
                }
            }
            else if (ch == 't' || ch == 'T')
            {
                if (app_mode == APP_MODE_STANDBY) {
                    ...
                    UART_DEBUG_PutString("[CLI] -> MOVI_TEST\r\n");
                    UART_DEBUG_PutString("  a=att  g=GPS  b=baro  s=stat  n=mag\r\n");
                    UART_DEBUG_PutString("  2=platform  4=motor  q=quat-xcheck  t=exit\r\n");
                } else {
                    UART_DEBUG_PutString("[CLI] Enter STANDBY first (m to stop ctrl)\r\n");
                }
            }
            else if (ch == 'r' || ch == 'R')
            {
                ...
                if (app_mode == APP_MODE_STANDBY) {
                    ...
                    UART_DEBUG_PutString("[RESET] Sending attr 382 = 0x01 (heading reset)\r\n");
                    UART_DEBUG_PutString("[RESET] Polling echo at 1 Hz.  x=abort\r\n");
                    UART_DEBUG_PutString("[RESET] Expected: GCU echoes 0x01 for ~10s, then 0x00 = done\r\n");
                    ...
                    UART_DEBUG_PutString("[CLI] Enter STANDBY first\r\n");
                }
            }
            else if (ch == 'v' || ch == 'V')
            {
                ...
                if (app_mode == APP_MODE_STANDBY) {
                    ...
                    UART_DEBUG_PutString("[SENSOR] Streaming: MAG, BARO, STAT, PLAT-ATT\r\n");
                    UART_DEBUG_PutString("[SENSOR] Rotate unit to observe yaw tracking.\r\n");
                    UART_DEBUG_PutString("[SENSOR] Any key to exit.\r\n");
                } else {
                    UART_DEBUG_PutString("[CLI] Enter STANDBY first\r\n");
                }
            }
            else if (ch == 'w' || ch == 'W')
            {
                if (app_mode == APP_MODE_STANDBY) {
                    ...
                    UART_DEBUG_PutString("[SCAN] Attr sweep, 500ms timeout, 200/batch\r\n");
                    UART_DEBUG_PutString("[SCAN] Format: attr,dt_ms,TX_hex,RX_hex\r\n");
                    UART_DEBUG_PutString("[SCAN] any key=next batch  x=abort\r\n");
                } else {
                    UART_DEBUG_PutString("[CLI] Enter STANDBY first\r\n");
                }
            }
            ...
            else if (ch == 'l' || ch == 'L')
            {
                ...
                if (ctrl_mode == ABSOLUTE)
                    UART_DEBUG_PutString("[CLI] ABS mode: keys 0-5 for angle presets\r\n");
            }
            else if (ch == 'k' || ch == 'K')
            {
                abs_preset_active = FALSE;
            ...
            else if (ch == 'x' || ch == 'X')
            {
                ...
                do_kill(&g_movi, &app_mode, &joystick_active,
                        &streaming_adc, &ctrl_mode);
                diag_active = FALSE;
                UART_DEBUG_PutString("[CLI] Output paused. Press m or j to resume.\r\n");
            }
            else if (ch == 's' || ch == 'S')
            {
            ...
            if (app_mode == APP_MODE_MOVI_CONTROL)
            {
                ...
                if (ch == 'f' || ch == 'F')
                {
                    ...
                    UART_DEBUG_PutString("[CTRL] Attr 48 requested (motor drive status)\r\n");
                }
        ...
        else if (app_mode == APP_MODE_MOVI_SCAN)
        {
            if (ch != 0u) {
                if (ch == 'x' || ch == 'X' || ch == 'k' || ch == 'K') {
                    ...
                    app_mode = APP_MODE_STANDBY;
                    UART_DEBUG_PutString("[SCAN] Aborted -> STANDBY\r\n");
        ...
        else if (app_mode == APP_MODE_MOVI_TEST)
        {
            if (ch == 't' || ch == 'T') {
                ...
                if (ch == 'x' || ch == 'X') {
                    diag_active = FALSE;
                    UART_DEBUG_PutString("[CLI] Output paused.\r\n");
                }
            }
            else if (ch == 'a' || ch == 'A') {
                movi_comm_request_attr(&g_movi, 22u);
                g_movi.test_sent_ms = g_tick_ms;
                UART_DEBUG_PutString("[TEST] Sent attr 22 (gimbal euler) ...\r\n");
            }
            else if (ch == 'g' || ch == 'G') {
                movi_comm_request_attr(&g_movi, 4u);
                g_movi.test_sent_ms = g_tick_ms;
                UART_DEBUG_PutString("[TEST] Sent attr 4 (GPS) ...\r\n");
            }
            else if (ch == 'b' || ch == 'B') {
                movi_comm_request_attr(&g_movi, 3u);
                g_movi.test_sent_ms = g_tick_ms;
                UART_DEBUG_PutString("[TEST] Sent attr 3 (baro) ...\r\n");
            }
            else if (ch == 's' || ch == 'S') {
                movi_comm_request_attr(&g_movi, 1u);
                g_movi.test_sent_ms = g_tick_ms;
                UART_DEBUG_PutString("[TEST] Sent attr 1 (system status) ...\r\n");
            }
            else if (ch == 'n' || ch == 'N') {
                movi_comm_request_attr(&g_movi, 12u);
                g_movi.test_sent_ms = g_tick_ms;
                UART_DEBUG_PutString("[TEST] Sent attr 12 (magnetometer) ...\r\n");
            }
            else if (ch == '2') {
                /* M2: platform body attitude */
                movi_comm_request_attr(&g_movi, 2u);
                g_movi.test_sent_ms = g_tick_ms;
                UART_DEBUG_PutString("[TEST] Sent attr 2 (platform attitude) ...\r\n");
                UART_DEBUG_PutString("[TEST] Rotate the whole unit to see yaw change.\r\n");
            }
            else if (ch == '4') {
                /* Test 4: motor drive status */
                movi_comm_request_attr(&g_movi, 48u);
                g_movi.test_sent_ms = g_tick_ms;
                UART_DEBUG_PutString("[TEST] Sent attr 48 (motor drive) ...\r\n");
                UART_DEBUG_PutString("[TEST] NOTE: off9-13 only non-zero if QX277 was active.\r\n");
                UART_DEBUG_PutString("[TEST] Enter CTRL (m) first, then test, then come back here.\r\n");
            }
            else if (ch == 'q' || ch == 'Q') {
                /* Test 3: quaternion cross-check */
        ...
        else if (app_mode == APP_MODE_RESET)
        {
            if (ch == 'x' || ch == 'X' || ch == 'k' || ch == 'K') {
                ...
                app_mode    = APP_MODE_STANDBY;
                UART_DEBUG_PutString("[RESET] Aborted -> STANDBY\r\n");
            }
        }
            ...
            if (scan_waiting_space) {
                ...
                if (scan_next_attr == scan_batch_start + SCAN_BATCH_SIZE
                    ...
                    if (scan_next_attr > SCAN_ATTR_MAX) {
                        UART_DEBUG_PutString("[SCAN] Complete -> STANDBY\r\n");
                        app_mode = APP_MODE_STANDBY;
                    } else {
        ...
        if (app_mode == APP_MODE_RESET)
        {
                ...
                g_movi.test_response_attr == 382u)
            {
                ...
                if (echo == 0x01u) {
                    ...
                    sprintf(tx, "[RESET] COMPLETE  t=%lus  echo=0x00\r\n",
                            (unsigned long)((g_tick_ms - reset_start_ms) / 1000u));
                    UART_DEBUG_PutString(tx);
                    UART_DEBUG_PutString("[RESET] Heading reference established.\r\n");
                    UART_DEBUG_PutString("[RESET] -> STANDBY.  Send QX277 ABSOLUTE to point.\r\n");
            ...
            if ((g_tick_ms - reset_start_ms) >= RESET_TIMEOUT_MS) {
                UART_DEBUG_PutString("[RESET] TIMEOUT (15s) -- GCU did not complete.\r\n");
                UART_DEBUG_PutString("[RESET] Check gimbal status flags, try again.\r\n");
                g_movi.test_pending_attr   = 0u;
                g_movi.test_response_ready = false;
                reset_state = RESET_IDLE;
                app_mode    = APP_MODE_STANDBY;
        ...
        if (last_button_state == 1 && current_pin_state == 0)
        {
            ...
            if (Pin_Sensitivity_Read() == 0) {
                ...
                UART_DEBUG_PutString("Mode Switched!\r\n");
            ...
            && (g_tick_ms - last_diag_ms) >= DIAG_PERIOD_MS)
        {
            ...
            if (st.valid) {
                ...
                UART_DEBUG_PutString("[Gimbal]  No valid frame yet\r\n");
            }
